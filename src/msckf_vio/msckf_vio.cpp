...
MsckfVio::MsckfVio(std::string dataSetConfig,std::string paramconfigfile,VioDrawer *pvioDrawer):
  is_gravity_set(false),
  is_first_img(true)
{
  this->mpVioDrawer = pvioDrawer;
  return;
}

bool MsckfVio::loadParameters(std::string dataSetConfig,std::string paramconfigfile) 
{
  // Frame id
  ...
  publish_tf = true;
  frame_rate = 20.0;
  cv::FileStorage fs(paramconfigfile, cv::FileStorage::READ);
  if (!fs.isOpened()) 
  {
      std::cout << "can't open parameter config file: "<< paramconfigfile << std::endl;
      return false;
  }
  cv::FileNode node0 = fs["global"];
  node0["position_std_threshold"] >> position_std_threshold;
  node0["rotation_threshold"]     >> rotation_threshold;
  node0["translation_threshold"]  >> translation_threshold;
  node0["tracking_rate_threshold"]>> tracking_rate_threshold;
  node0["max_cam_state_size"]     >> max_cam_state_size;

  // Feature optimization parameters
  cv::FileNode node1 = fs["feature"];
  node1["translation_threshold"] >> Feature::optimization_config.translation_threshold;

  // Noise related parameters
  cv::FileNode node2 = fs["noise"];
  node2["gyro"] >> IMUState::gyro_noise;
  node2["acc"]  >> IMUState::acc_noise;
  node2["gyro_bias"] >> IMUState::gyro_bias_noise;
  node2["acc_bias"]  >> IMUState::acc_bias_noise;
  node2["feature"]   >> Feature::observation_noise;
  ...
  cv::FileNode node3 = fs["initial_state_velocity"];
  node3["x"] >> state_server.imu_state.velocity(0);
  node3["y"] >> state_server.imu_state.velocity(1);
  node3["z"] >> state_server.imu_state.velocity(2);

  // The initial covariance of orientation and position can be
  // set to 0. But for velocity, bias and extrinsic parameters,
  // there should be nontrivial uncertainty.
  double gyro_bias_cov, acc_bias_cov, velocity_cov;
  cv::FileNode node4 = fs["initial_covariance"];
  node4["velocity"]  >> velocity_cov;
  node4["gyro_bias"] >> gyro_bias_cov;
  node4["acc_bias"]  >> acc_bias_cov;

  double extrinsic_rotation_cov, extrinsic_translation_cov;
  node4["extrinsic_rotation_cov"] >> extrinsic_rotation_cov;
  node4["extrinsic_translation_cov"] >> extrinsic_translation_cov;

  initial_gyro_bias_cov = gyro_bias_cov;
  initial_acc_bias_cov = acc_bias_cov;
  initial_velocity_cov = velocity_cov;
  initial_extrinsic_rotation_cov = extrinsic_rotation_cov;
  initial_extrinsic_translation_cov = extrinsic_translation_cov;
  ...
  cv::FileStorage fs_calib(dataSetConfig, cv::FileStorage::READ);
  if(!fs_calib.isOpened())
  {
      std::cout << "can't open calibration file: "<< dataSetConfig << std::endl;
      return false;
  }

  cv::FileNode node5 = fs_calib["cam0"];
  vector<double> vT_cam_imu_tmp;
  node5["T_cam_imu"] >> vT_cam_imu_tmp;
  cv::Mat T_cam_imu_tmp(4,4,CV_64F,&vT_cam_imu_tmp[0]);
  Isometry3d T_imu_cam0 = utils::getTransformEigen(T_cam_imu_tmp);
  Isometry3d T_cam0_imu = T_imu_cam0.inverse();

  state_server.imu_state.R_imu_cam0 = T_cam0_imu.linear().transpose();
  state_server.imu_state.t_cam0_imu = T_cam0_imu.translation();

  cv::FileNode node6 = fs_calib["cam1"];
  vector<double> vT_cam0_cam1;
  node6["T_cn_cnm1"] >> vT_cam0_cam1;
  cv::Mat T_cam0_cam1_tmp(4,4,CV_64F,&vT_cam0_cam1[0]);
  CAMState::T_cam0_cam1 = utils::getTransformEigen(T_cam0_cam1_tmp);
  
  cv::FileNode node7 = fs_calib["T_imu_body"];
  vector<double> vT_imu_body;
  node7 >> vT_imu_body;
  cv::Mat vT_imu_body_tmp(4,4,CV_64F,&vT_imu_body[0]);
  IMUState::T_imu_body = utils::getTransformEigen(vT_imu_body_tmp).inverse();
  ...
  std::cout <<"R_imu_cam0 = "<<std::endl<< T_imu_cam0.linear() << endl;
  std::cout <<"t_imu_cam0 = "<<std::endl<< T_imu_cam0.translation().transpose() << endl;
...
bool MsckfVio::initialize(std::string dataSetConfig,std::string paramconfigfile) {
  if (!loadParameters(dataSetConfig,paramconfigfile)) return false;
...
bool MsckfVio::imuCallback(std::vector<IMU> &vImu) 
{
  //将IMU数据放入列表中
  ...
  this->imu_buffer.insert(this->imu_buffer.end(),vImu.begin(),vImu.end());
  if (!is_gravity_set) 
  {
...
bool MsckfVio::resetCallback() 
{
  ...
  state_server.state_cov = MatrixXd::Zero(21, 21);
  for (int i = 3; i < 6; ++i)
    state_server.state_cov(i, i) = initial_gyro_bias_cov;
  for (int i = 6; i < 9; ++i)
    state_server.state_cov(i, i) = initial_velocity_cov;
  for (int i = 9; i < 12; ++i)
    state_server.state_cov(i, i) = initial_acc_bias_cov;
  for (int i = 15; i < 18; ++i)
    state_server.state_cov(i, i) = initial_extrinsic_rotation_cov;
  for (int i = 18; i < 21; ++i)
    state_server.state_cov(i, i) = initial_extrinsic_translation_cov;
...
void MsckfVio::featureCallback(
    const Frame_Feature& frameFeature) {
  ...
  {
    is_first_img = false;
    state_server.imu_state.time = frameFeature.stamp;
  }
  ...
  batchImuProcessing(frameFeature.stamp);
  double imu_processing_time = (std::chrono::steady_clock::now()-start_time).count();

  // Augment the state vector.
  start_time = std::chrono::steady_clock::now();
  stateAugmentation(frameFeature.stamp);
  double state_augmentation_time = (std::chrono::steady_clock::now()-start_time).count();

  // Add new observations for existing features or new
  // features in the map server.
  start_time = std::chrono::steady_clock::now();
  addFeatureObservations(frameFeature);
  double add_observations_time = (std::chrono::steady_clock::now()-start_time).count();

  // Perform measurement update if necessary.
  start_time = std::chrono::steady_clock::now();
  removeLostFeatures();
  double remove_lost_features_time = (std::chrono::steady_clock::now()-start_time).count();

  start_time = std::chrono::steady_clock::now();
  pruneCamStateBuffer();
  double prune_cam_states_time = (std::chrono::steady_clock::now()-start_time).count();;

  // Publish the odometry.
  // start_time = ros::Time::now();
  // publish(msg->header.stamp);
  this->mpVioDrawer->UpdateMap(this->state_server,this->map_server);
  // double publish_time = (
  ...
  {
    ++critical_time_cntr;
    printf("Total processing time %f/%d",processing_time, critical_time_cntr);
...
void MsckfVio::batchImuProcessing(const double& time_bound) {
  ...
  imu_buffer.erase(imu_buffer.begin(),
      imu_buffer.begin()+used_imu_msg_cntr);
...
void MsckfVio::addFeatureObservations(
    const Frame_Feature& frameFeatures) {
  ...
  // Add new observations for existing features or new
  // features in the map server.
  for (const auto& feature : frameFeatures.features) {
