...
namespace msckf_vio 
{
ImageProcessor::ImageProcessor(std::string dataSetConfig,std::string paramconfigfile) :
  is_first_img(true),
  //img_transport(n),
  // stereo_sub(10),
...
bool ImageProcessor::loadParameters(std::string calib_file,std::string paramconfigfile) 
{
   cv::FileStorage fs(calib_file, cv::FileStorage::READ);
   if (!fs.isOpened()) 
   {
       std::cout << "can't open calibration file: "<< calib_file << std::endl;
       return false;
   }

    // Camera0 calibration parameters
    cv::FileNode node0 = fs["cam0"];
    if (node0.empty()) 
    {
        cam0_distortion_model = std::string("radtan");
    }
    else 
    {
        node0["distortion_model"] >> cam0_distortion_model;
    }
    // Camera calibration parameters
    vector<int> cam0_resolution_temp(2);
    node0["resolution"] >> cam0_resolution_temp;
    cam0_resolution[0] = cam0_resolution_temp[0];
    cam0_resolution[1] = cam0_resolution_temp[1];

    vector<double> cam0_intrinsics_temp(4);
    node0["intrinsics"] >> cam0_intrinsics_temp;
    ...
    node0["distortion_coeffs"] >> cam0_distortion_coeffs_temp;
    ...
    node0["T_cam_imu"] >> T_imu_cam0_temp;//imu转到cam
    ...
    cv::FileNode node1 = fs["cam1"];
    if (node1.empty()) 
    {
        cam1_distortion_model = std::string("radtan");
    }
    else 
    {
        node1["distortion_model"] >> cam1_distortion_model;
    }

    vector<int> cam1_resolution_temp(2);
    node1["resolution"] >> cam1_resolution_temp;
    ...
    node1["intrinsics"] >> cam1_intrinsics_temp;
    ...
    node1["distortion_coeffs"] >> cam1_distortion_coeffs_temp;
    ...
    node1["T_cn_cnm1"] >> T_imu_cam1_temp;//imu转到cam
    cv::Mat T_cam0_cam1(4,4,CV_64F,&T_imu_cam1_temp[0]);
    ...
    cv::FileStorage fconfig(paramconfigfile, cv::FileStorage::READ);
    if(!fconfig.isOpened()) 
    {
       std::cout << "can't open parameter Configure file: "<< paramconfigfile << std::endl;
       return false;
    }

    cv::FileNode node2 = fconfig["image_processor"];
    node2["grid_row"] >> processor_config.grid_row;
    node2["grid_col"] >> processor_config.grid_col;
    node2["grid_min_feature_num"] >> processor_config.grid_min_feature_num;
    node2["grid_max_feature_num"] >> processor_config.grid_max_feature_num;
    node2["pyramid_levels"] >> processor_config.pyramid_levels;
    node2["patch_size"] >> processor_config.patch_size;
    node2["fast_threshold"] >> processor_config.fast_threshold;
    node2["max_iteration"] >> processor_config.max_iteration;
    node2["track_precision"] >> processor_config.track_precision;
    node2["ransac_threshold"] >> processor_config.ransac_threshold;
    node2["stereo_threshold"] >> processor_config.stereo_threshold;
  ...
  std::cout << "R_imu_cam0"<<std::endl<<R_imu_cam0 << endl;
  std::cout << "t_imu_cam0.t() "<<std::endl<<t_imu_cam0.t() << endl;
...
bool ImageProcessor::initialize(std::string calib_file,std::string paramconfigfile) 
{
  if (!loadParameters(calib_file,paramconfigfile)) 
  {
     return false;
  }
  ...
  printf("Finish creating Detector...\n");

  return true;
}

void ImageProcessor::stereoCallback(
    const ImageFrame& cam0_img,
    const ImageFrame& cam1_img) {

  //cout << "==================================" << endl;

  // Get the current image.
  cam0_curr_img = cam0_img;
  cam1_curr_img = cam1_img;
...
{
  const Mat& curr_cam0_img = cam0_curr_img.image;
  ...
  const Mat& curr_cam1_img = cam1_curr_img.image;
  buildOpticalFlowPyramid(
...
{
  // Size of each grid.
  const Mat& img = cam0_curr_img.image;
...
void ImageProcessor::trackFeatures() {
  ...
  static int grid_height =
    cam0_curr_img.image.rows / processor_config.grid_row;
  static int grid_width =
    cam0_curr_img.image.cols / processor_config.grid_col;
  ...
  for (int i = 0; i < curr_cam0_points.size(); ++i) {
    ...
    if (curr_cam0_points[i].y < 0 ||
        curr_cam0_points[i].y > cam0_curr_img.image.rows-1 ||
        curr_cam0_points[i].x < 0 ||
        curr_cam0_points[i].x > cam0_curr_img.image.cols-1)
      track_inliers[i] = 0;
  }
  ...
  printf("candidates: %d; track: %d; match: %d; ransac: %d/%d=%f\n",
...
void ImageProcessor::stereoMatch(
  ...
  for (int i = 0; i < cam1_points.size(); ++i) {
    ...
    if (cam1_points[i].y < 0 ||
        cam1_points[i].y > cam1_curr_img.image.rows-1 ||
        cam1_points[i].x < 0 ||
        cam1_points[i].x > cam1_curr_img.image.cols-1)
      inlier_markers[i] = 0;
  }
...
void ImageProcessor::addNewFeatures() {
  const Mat& curr_img = cam0_curr_img.image;
...
void ImageProcessor::integrateImuData(
  ...
  double dtime = (cam0_curr_img.stamp-
      cam0_prev_img.stamp);
...
void ImageProcessor::twoPointRansac(
  ...
  std::vector<int> vAvailableIndexs(raw_inlier_idx.size()-1);
  for(int id = 0; id < vAvailableIndexs.size();id++)
  {
    vAvailableIndexs[id] = id;
  }
  ...
  for (int iter_idx = 0; iter_idx < iter_num; ++iter_idx) 
  {
    ...
    std::vector<int> vTempAvailableIndexs = vAvailableIndexs;
    int select_idx1 = DUtils::Random::RandomInt(0,vTempAvailableIndexs.size()-1);
    vTempAvailableIndexs[select_idx1] = vTempAvailableIndexs.back();
    int select_idx_diff = DUtils::Random::RandomInt(0,vTempAvailableIndexs.size()-2);
...
void ImageProcessor::publish() {
  ...
  // Publish features.
  mFramefeature.stamp = cam0_curr_img.stamp;
  ...
  {
    Stereo_Feature_Pt pt;
    mFramefeature.features.push_back(pt);
    mFramefeature.features[i].id = curr_ids[i];
    mFramefeature.features[i].u0 = curr_cam0_points_undistorted[i].x;
    mFramefeature.features[i].v0 = curr_cam0_points_undistorted[i].y;
    mFramefeature.features[i].u1 = curr_cam1_points_undistorted[i].x;
    mFramefeature.features[i].v1 = curr_cam1_points_undistorted[i].y;
  }
...
void ImageProcessor::drawFeaturesMono() {
  ...
  int img_height = cam0_curr_img.image.rows;
  int img_width = cam0_curr_img.image.cols;
  Mat out_img(img_height, img_width, CV_8UC3);
  cvtColor(cam0_curr_img.image, out_img, CV_GRAY2RGB);
...
void ImageProcessor::drawFeaturesStereo() {
  ...
  // if(debug_stereo_pub.getNumSubscribers() > 0)
  {
    ...
    static int grid_height = cam0_curr_img.image.rows / processor_config.grid_row;
    static int grid_width = cam0_curr_img.image.cols / processor_config.grid_col;

    // Create an output image.
    int img_height = cam0_curr_img.image.rows;
    int img_width = cam0_curr_img.image.cols;
    Mat out_img(img_height, img_width*2, CV_8UC3);
    cvtColor(cam0_curr_img.image,out_img.colRange(0, img_width), CV_GRAY2RGB);
    cvtColor(cam1_curr_img.image,out_img.colRange(img_width, img_width*2), CV_GRAY2RGB);
...
void ImageProcessor::AddImu(std::vector<IMU> &vIMUs)
{
  imu_buffer.insert(imu_buffer.end(),vIMUs.begin(),vIMUs.end());
}
