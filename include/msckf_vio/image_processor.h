...
public:
  // Constructor
  ImageProcessor(std::string dataSetConfig,std::string paramconfigfile);
  ...
  bool initialize(std::string calib_file,std::string paramconfigfile);
...
private:
   ...
   */
  bool loadParameters(std::string calib_file,std::string paramconfigfile);
  ...
  ImageFrame cam0_prev_img;
  ImageFrame cam0_curr_img;
  ImageFrame cam1_curr_img;
...
public:
   ...
   */
  void stereoCallback(const ImageFrame& cam0_img,const ImageFrame& cam1_img);
  void AddImu(std::vector<IMU> &vIMUs);
  Frame_Feature mFramefeature;
};
